...
def login(request,handler):
    ...
    secret = open(os.path.join("private.key"), 'rb').read()
...
def logout(request,handler):
    ...
    userjwt = request.cookies.get('jwt')
    res.cookies({'jwt':userjwt + '; HttpOnly; Secure; Max-Age=0'})
...
def user(request,handler):
    res = Response()
    if 'auth_token' not in request.cookies or not token_collection.find_one({'auth_token':hashlib.sha256(request.cookies.get('auth_token').encode()).hexdigest()}):
        res.set_status(401,'Unauthorized')
        res.json({})
        handler.request.sendall(res.to_data())
        return
